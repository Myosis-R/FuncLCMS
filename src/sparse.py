...
class Grid2D:
    ...
    def __init__(self, data, coord0, coord1,
                 axis_names=("axis0", "axis1"),
                 copy=False):
        """
        Parameters
        ----------
    ...
    def copy(self, deep=True):
        ...
        return Grid2D(data, coord0, coord1,
    ...
    def window(self, axis, min_val, max_val, *, inplace=False):
        ...
        if start >= end:
            # Empty window
            new_data = sp.csr_array((0, self.shape[1-axis])) if axis == 0 \
        ...
        return Grid2D(new_data, new_coords[0], new_coords[1],
                      axis_names=self.axis_names)
    ...
    def interpolate_axis(self, axis, out_coord, in_coord=None, *, inplace=False):
        ...
        mask = np.logical_and(out_coord < np.max(in_coord),
                              out_coord > np.min(in_coord))
        ...
        coefs = ((out_coord - in_coord[idxs - 1]) /
                 (in_coord[idxs] - in_coord[idxs - 1]))
        ...
        return Grid2D(new_ds, new_coords[0], new_coords[1],
                      axis_names=self.axis_names)
    ...
    def sum(self, axis=None):
        """
        Sum over an axis or both axes.

        Parameters
        ----------
        axis : {None, 0, 1}, optional
            If None, sum over both axes and return a scalar.
            If 0 or 1, sum along that axis and return a 1D numpy array.
        """
        if axis is None:
            return self._data.sum()
        if axis == 0:
            # sum over rows -> 1 value per column
            return np.asarray(self._data.sum(axis=0)).ravel()
        if axis == 1:
            # sum over columns -> 1 value per row
            return np.asarray(self._data.sum(axis=1)).ravel()
        raise ValueError("axis must be None, 0 or 1")

    def tic(self, axis=0, boolean=False):
        """
        Total Ion Chromatogram-like trace along one axis.

        Parameters
        ----------
        axis : {0, 1}
            Axis to keep (i.e. sum over 1-axis).
            For typical spectra: axis=0 -> TIC over tmz as function of rt.
        ...
        if boolean:
            ...
            # We use coo to access row/col indices easily.
            coo = self._data.tocoo()
            data_bool = sp.coo_array(
                (np.ones_like(coo.data, dtype=float),
                 (coo.row, coo.col)),
                shape=coo.shape
            )
            if axis == 0:
                trace = np.asarray(data_bool.sum(axis=1)).ravel()
            else:
    ...
    def _binary_op(self, other, op):
        if isinstance(other, Grid2D):
            if not self.is_compatible_with(other):
                raise ValueError("Grid2D objects have different coordinates "
                                 "or shapes; cannot combine safely.")
            new_data = op(self._data, other._data)
            return Grid2D(new_data, self.coord0, self.coord1,
                          axis_names=self.axis_names)
        else:
            # scalar
    ...
    def __repr__(self):
        return (f"Grid2D(shape={self.shape}, nnz={self.nnz}, "
